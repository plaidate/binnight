#!/bin/bash
# Bin Night smoke runner: build instrumented, run headless in the Playdate
...
SHOT="$ROOT/build/binnight-shot.png"
...
rm -rf "$DATA" "$SHOT"
("$SIM" "$ROOT/out/BinNightSmoke.pdx" >"$ROOT/build/sim.log" 2>&1 &)
